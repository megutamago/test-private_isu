...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	row := db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` ORDER BY `created_at` DESC")
	err = row.Scan(&result.ID, &result.Mime)
	if err != nil {
			log.Print(err)
			return
			//return nil, err
...
func getUser(id string, db *sqlx.DB) (*User, error) {
	...
	user := &User{}
	err = row.Scan(&user.ID, &user.AccountName)
	if err != nil {
			return nil, err
	}
